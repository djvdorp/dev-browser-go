...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="dev-browser", add_help=True)
    parser.add_argument("--profile", default=os.environ.get("DEV_BROWSER_PROFILE", "default"))
    parser.add_argument("--headless", action="store_true", default=os.environ.get("HEADLESS", "").lower() in {"1", "true", "yes"})
    parser.add_argument("--output", choices=["summary", "json", "path"], default="summary")
    parser.add_argument("--out", default="", help="When --output=path, optional relative path under artifact dir.")

    sub = parser.add_subparsers(dest="cmd", required=True)

    sub.add_parser("status", help="Show daemon status for the profile.")
    sub.add_parser("start", help="Start daemon (if not running).")
    sub.add_parser("stop", help="Stop daemon (if running).")
    sub.add_parser("list-pages", help="List named pages managed by the daemon.")

    p_call = sub.add_parser("call", help="Call a tool by name with JSON args.")
    p_call.add_argument("tool")
    p_call.add_argument("--args", default="{}")
    p_call.add_argument("--page", default="main")

    p_goto = sub.add_parser("goto", help="Navigate a page to a URL.")
    ...
    p_snapshot = sub.add_parser("snapshot", help="Get a token-light snapshot (refs).")
    ...
    p_click = sub.add_parser("click-ref", help="Click a ref from the latest snapshot.")
    p_click.add_argument("ref")
    p_click.add_argument("--page", default="main")
    p_click.add_argument("--timeout-ms", type=int, default=15000)

    p_fill = sub.add_parser("fill-ref", help="Fill a ref from the latest snapshot.")
    ...
    p_press = sub.add_parser("press", help="Send a key press to the page.")
    p_press.add_argument("key")
    p_press.add_argument("--page", default="main")

    p_shot = sub.add_parser("screenshot", help="Save a screenshot and print path.")
    ...
    p_html = sub.add_parser("save-html", help="Save page HTML and print path.")
    p_html.add_argument("--page", default="main")
    p_html.add_argument("--path", default="")

    p_actions = sub.add_parser("actions", help="Run a batch of calls (JSON).")
    ...
    p_wait = sub.add_parser("wait", help="Wait for a load state (non-throwing on timeout).")
    ...
    p_close = sub.add_parser("close-page", help="Close a named page.")
    ...
    if args.cmd == "screenshot":
        ...
        with open_page(conn["wsEndpoint"], conn["targetId"]) as (_browser, _context, page):
            result = run_call(
                page,
                "screenshot",
                {"path": args.path or None, "full_page": bool(args.full_page), "annotate_refs": bool(args.annotate_refs)},
                artifact_dir=artifact_dir(profile),
            )
        write_output(profile, output_mode, result, out_path)
        return 0
