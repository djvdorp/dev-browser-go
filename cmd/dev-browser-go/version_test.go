...
func TestVersionFromBuildInfoPrefersBaseVersionWhenRevisionPresent(t *testing.T) {
	...
	if got := versionFromBuildInfo(baseVersion, info); got != "0.2.3+ga38dae312345" {
		t.Fatalf("versionFromBuildInfo() = %q, want %q", got, "0.2.3+ga38dae312345")
	}
}
...
func TestVersionFromBuildInfoAddsDirtyMarker(t *testing.T) {
	...
	if got := versionFromBuildInfo(baseVersion, info); got != "0.2.3+ga38dae312345.dirty" {
		t.Fatalf("versionFromBuildInfo() = %q, want %q", got, "0.2.3+ga38dae312345.dirty")
	}
}
